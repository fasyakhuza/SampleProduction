...
def create_jobs(input_rootfiles):
	run_dir = os.path.join(os.getcwd(), 'SIM_Condor_input_'+str(input_rootfiles).replace('.txt',''))
	os.mkdir(run_dir)
	store_path="/eos/cms/store/user/tsarkar/"
	dir_name=store_path+"SIM_Condor_output_"+str(input_rootfiles).replace('.txt','')
	os.mkdir(dir_name)
	...
	for i in range(len(filename)):
		print(filename[i])
		sim_cfg_file="sim_run_"+str(i+1)+".py"
		infile= filename[i]
		outfile="step_sim_2017UL_"+str(i+1)+".root"
